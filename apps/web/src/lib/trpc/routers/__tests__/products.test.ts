...
describe("products.delete", () => {
	...
	it("deletes a product referenced by order items without deleting history", async () => {
		const p = await caller.create({
			name: "Sold Product",
			price: 100,
			in_stock: 1,
		...
		expect(persistedItem.product_id).toBeNull();
		expect(persistedItem.price).toBe(100);
	});
});
