...
function processDir(dir: string) {
	for (const entry of readdirSync(dir, { withFileTypes: true })) {
		...
		let title = headingMatch[1]
			.replace(
				/^(Class|Interface|Function|Type Alias|Variable|Enumeration):\s*/,
				"",
			)
			.trim();
...
console.log(
	`Done — ${totalEn} EN + ${totalPt} pt-BR pages (${descriptionMap.size} translated descriptions)`,
);
